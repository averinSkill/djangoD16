...
def confirmation_code(request):
    ...
    send_mail(
        ...
        message=f'Здравствуйте, {request.user}!'
                f'Cкопируйте следующий код {user.code}'
                f'и вставьте в форму подтверждения на странице\nhttp://127.0.0.1:8000/sign/profile',
    ...
    def form_valid(self, form, **kwargs):
        ...
        if form.cleaned_data['code'] == CustomUser.objects.get(user=self.request.user).code:
            ...
            if not self.request.user.groups.filter(name='authors').exists():
                print(f"USER {self.request.user} успешно зареган")
                premium_group.user_set.add(self.request.user)
        else:
